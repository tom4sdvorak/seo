...
module ExtractImages
  ...
  def initialize (pathname)
    ...
    def process_page(page, count)
      ...
      return saved_images if xobjects.empty?
      xobjects.each do |name, stream|
        ...
        when :Image then
          ...
          when :CCITTFaxDecode then 
            ExtractImages::Tiff.new(stream).save( @pathname.join("#{page.number}-#{count}-#{name}.tif") )
            saved_images << "#{page.number}-#{count}-#{name}.tif"
          when :DCTDecode      then
            ExtractImages::Jpg.new(stream).save( @pathname.join("#{page.number}-#{count}-#{name}.jpg") )
            saved_images << "#{page.number}-#{count}-#{name}.jpg"
          else
            ExtractImages::Raw.new(stream).save( @pathname.join("#{page.number}-#{count}-#{name}.tif") )
            saved_images << "#{page.number}-#{count}-#{name}.tif"
          end
        when :Form then
          process_page(PDF::Reader::FormXObject.new(page, stream), count)
        end
      end
      saved_images
    end
