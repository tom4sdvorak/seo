...
val="$?"
if [ $val -eq 127 ] ; then
	echo "$escape[01;32mFAIL:$escape[00m Ruby not installed. Please install it."
	exit 1
else
	...
	if [ $correct -eq 0 ] ; then
		echo "$escape[01;32mFAIL:$escape[00m Your Ruby version is too low ( >= 2.1 is needed)"
		exit 1
	else
...
if [ $val -eq 127 ] ; then
	echo "$escape[01;32mFAIL:$escape[00m RubyGems not installed. Please install it."
	exit 1
else
	version=`gem -v`
...
val="$?"
if [ $val -eq 127 ] ; then
	echo "$escape[01;32mFAIL:$escape[00m Node.js not installed. Please install it."
	exit 1
else
...
val="$?"
if [ $val -eq 127 ] ; then
	echo "$escape[01;32mFAIL:$escape[00m npm not installed. Please install it."
	exit 1
else
...
if [ $val -eq 0 ] ; then
	...
	echo "$escape[01;32mFAIL:$escape[00m The pdf-reader gem is not installed. Please install it. (or RubyGems is unreachable)"
